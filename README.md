...
A month-by-month model of one company's operating plan. It turns strategy into work, work into hours, and hours into people, then shows where the plan runs out of people, when, and what would change that. Every number is computed by a deterministic engine from one JSON file. Nothing on the pages is written in as a conclusion.
