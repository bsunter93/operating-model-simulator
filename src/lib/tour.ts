...
export const TOUR: TourStep[] = [
  ...
  },
  {
    ...
    body: (c) => {
      ...
      const where = rank === 1 ? 'It is also the most expensive constraint.' : rank > 1 ? `By cost it ranks ${rank}${costlier.length ? `: ${costlier.map((x) => c.teamName(x.teamId!)).join(' and ')} ${costlier.length === 1 ? 'costs' : 'cost'} more but ${costlier.length === 1 ? 'breaks' : 'break'} later` : ''}.` : '';
